...
public class MinesweeperGame {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (doesUserChooseToPlantFlag(userActionInput)) {
                ...
                if (isLandMineCell(selectedRowIndex, selectedColIndex)) { // 지뢰 셀이라면
    ...
    private static void checkIfGameIsOver() {
        ...
        if (isAllOpened) { // 다 열려있다면 게임이 끝난 것이니까
    ...
    private static boolean isAllCellIsOpened() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                if (BOARD[row][col].equals(CLOSED_CELL_SIGN)) { // 지뢰판이 열려있지 않은게 있다면
                    isAllOpened = false;
                }
            }
        }
    ...
    private static void initializeGame() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                int count = 0;
                if (!isLandMineCell(row, col)) { // 지뢰인 칸은 true, 아닌 칸은 false?
                    if (row - 1 >= 0 && col - 1 >= 0 && isLandMineCell(row - 1, col - 1)) { // 현재 칸 기준 왼쪽 대각 위가 지뢰가 있다면?
                        count++;
                    }
